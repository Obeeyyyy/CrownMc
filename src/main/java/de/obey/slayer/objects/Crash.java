...
import de.obey.slayer.handler.CrashHandler;
...
public final class Crash {
    ...
    private void startCrash() {
        ...
        }.runTaskTimer(SlayerMain.getInstance(), 1, 1);
    }
    ...
    private void endCrash() {
        ...
        bets.keySet().forEach(uuid -> {
            final Player player = Bukkit.getPlayer(uuid);
            if (player != null && player.isOnline()) {
                SlayerMain.getInstance().getInitializer().getDailyPotHandler().addMoney(bets.get(uuid)/20);
        ...
        }.runTaskLater(SlayerMain.getInstance(), 20 * 5);
    }
